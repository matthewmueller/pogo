...
func FindMany(db jack.DB, condition string, params ...interface{}) ([]*Report, error) {
	var _o []*Report
...
func FindOne(db jack.DB, condition string, params ...interface{}) (*Report, error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
	SELECT "id", "user_id", "timestamp", "questions", "standup_id", "status", "created_at", "updated_at"
	FROM jack.reports
...
func UpdateMany(db jack.DB, report *Report, condition string, params ...interface{}) ([]*Report, error) {
	var _o []*Report

	// prepare the slices
	_c, _i, _v := jack.Slice(getColumns(report), len(params))
