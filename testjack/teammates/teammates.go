...
func (teammate *Teammate) ID(id uuid.UUID) *Teammate {
	teammate.columns.ID = testjack.DecodeUUID(id)
	return teammate
}

// GetID returns the `id` if set
func (teammate *Teammate) GetID() (id *uuid.UUID) {
	return testjack.EncodeUUID(teammate.columns.ID)
}
...
func Update(db testjack.DB, id uuid.UUID, teammate *Teammate) (*Teammate, error) {
	_id := testjack.DecodeUUID(id)
...
func UpdateBySlackID(db testjack.DB, slackID string, teammate *Teammate) (*Teammate, error) {
	fields := getColumns(teammate)
