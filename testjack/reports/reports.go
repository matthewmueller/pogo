...
func Insert(db testjack.DB, reportInput *ReportInput) (*Report, error) {
	// get all the non-nil columns and prepare them for the query
	_c, _i, _v := testjack.Slice(getColumns(reportInput), 0)
...
func FindByID(db testjack.DB, id string) (*Report, error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
    SELECT "id", "user_id", "timestamp", "questions", "standup_id", "status", "created_at", "updated_at"
    FROM "jack.reports"
...
func FindMany(db testjack.DB, where *WhereClause) (reports []*Report, err error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
  SELECT "id", "user_id", "timestamp", "questions", "standup_id", "status", "created_at", "updated_at"
  FROM "jack.reports"
...
func Update(db testjack.DB, id string, reportInput *ReportInput) (*Report, error) {
...
func UpdateMany(db testjack.DB, where *WhereClause, reportInput *ReportInput) (reports []*Report, err error) {
	// prepare the slices
	_c, _i, _v := testjack.Slice(getColumns(reportInput), len(where.params))
...
func Upsert(db testjack.DB, reportInput *ReportInput) (*Report, error) {
	// get all the non-nil columns and prepare them for the query
	_c, _i, _v := testjack.Slice(getColumns(reportInput), 0)
