...
var ErrCronNotFound = errors.New("crons not found")
...
func Insert(db digby.DB, cronInput *CronInput) (*Cron, error) {
	// get all the non-nil columns and prepare them for the query
	_c, _i, _v := digby.Slice(getColumns(cronInput), 0)
...
func FindByID(db digby.DB, id string) (*Cron, error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
    SELECT "id", "creator_id", "run_every", "run_url", "last_run_at", "created_at", "updated_at"
    FROM "public"."crons"
...
func FindOne(db digby.DB, where *WhereClause) (*Cron, error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
  SELECT "id", "creator_id", "run_every", "run_url", "last_run_at", "created_at", "updated_at"
  FROM "public"."crons"
...
func FindMany(db digby.DB, where *WhereClause) (crons []*Cron, err error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
  SELECT "id", "creator_id", "run_every", "run_url", "last_run_at", "created_at", "updated_at"
  FROM "public"."crons"
...
func Update(db digby.DB, id string, cronInput *CronInput) (*Cron, error) {
...
func UpdateMany(db digby.DB, where *WhereClause, cronInput *CronInput) (crons []*Cron, err error) {
	// prepare the slices
	_c, _i, _v := digby.Slice(getColumns(cronInput), len(where.params))
...
func Upsert(db digby.DB, cronInput *CronInput) (*Cron, error) {
	// get all the non-nil columns and prepare them for the query
	_c, _i, _v := digby.Slice(getColumns(cronInput), 0)
