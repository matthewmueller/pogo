...
func FindMany(db digby.DB, where *WhereClause) (activities []*Activity, err error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
  SELECT "id", "project_id", "creator_id", "subject", "icon", "event_count", "read_at", "created_at", "updated_at"
  FROM "public"."activities"
...
func UpdateMany(db digby.DB, where *WhereClause, activityInput *ActivityInput) (activities []*Activity, err error) {
	// prepare the slices
