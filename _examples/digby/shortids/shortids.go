...
func FindMany(db digby.DB, where *WhereClause) (shortids []*Shortid, err error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
  SELECT "id", "created_at"
  FROM "public"."shortids"
...
func UpdateMany(db digby.DB, where *WhereClause, shortidInput *ShortidInput) (shortids []*Shortid, err error) {
	// prepare the slices
