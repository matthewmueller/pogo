...
func FindMany(db digby.DB, where *WhereClause) (users []*User, err error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
  SELECT "id", "first_name", "last_name", "email", "password", "confirmed_at", "created_at", "updated_at"
  FROM "public"."users"
...
func UpdateMany(db digby.DB, where *WhereClause, userInput *UserInput) (users []*User, err error) {
	// prepare the slices
