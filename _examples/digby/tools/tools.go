...
func FindMany(db digby.DB, where *WhereClause) (tools []*Tool, err error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
  SELECT "id", "name", "icon", "tool_url", "activity_url", "event_url", "created_at", "updated_at"
  FROM "public"."tools"
...
func UpdateMany(db digby.DB, where *WhereClause, toolInput *ToolInput) (tools []*Tool, err error) {
	// prepare the slices
