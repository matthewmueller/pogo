...
func FindMany(db jack.DB, where *WhereClause) (standups []*Standup, err error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
  SELECT "id", "name", "slack_channel_id", "time", "timezone", "questions", "team_id", "created_at", "updated_at"
  FROM "jack"."standups"
...
func UpdateMany(db jack.DB, where *WhereClause, standupInput *StandupInput) (standups []*Standup, err error) {
	// prepare the slices
