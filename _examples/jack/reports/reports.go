...
func FindMany(db jack.DB, where *WhereClause) (reports []*Report, err error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
  SELECT "id", "user_id", "timestamp", "questions", "standup_id", "status", "created_at", "updated_at"
  FROM "jack"."reports"
...
func UpdateMany(db jack.DB, where *WhereClause, reportInput *ReportInput) (reports []*Report, err error) {
	// prepare the slices
