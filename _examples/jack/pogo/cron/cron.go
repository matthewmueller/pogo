...
var _ pogo.Row = (*Cron)(nil)

// Decode implements pogo.Row
func (c *Cron) Decode(cron interface{}) error {
	_cron, ok := cron.(*Cron)
	if !ok {
		return fmt.Errorf(`cron decode: "%T" is not a "*Cron"`, cron)
	}
	*_cron = *c
	return nil
}
...
// NullableFrequency nullable frequency equals
func (f *Filter) NullableFrequency(v *string) *Filter {
	if v == nil {
		f.formats = append(f.formats, `"frequency" IS NULL`)
		return f
	}
	f.formats = append(f.formats, `"frequency" = %s`)
	f.values = append(f.values, v)
...
// NullableFrequencyNot nullable frequency is not equal
func (f *Filter) NullableFrequencyNot(v *string) *Filter {
	if v == nil {
		f.formats = append(f.formats, `"frequency" IS NOT NULL`)
		return f
	}
	f.formats = append(f.formats, `"frequency" != %s`)
	f.values = append(f.values, v)
...
// InsertMany "crons" into the "jack"."crons"
func InsertMany(db pogo.DB, crons ...*Input) ([]*Cron, error) {
	_crons := []*Cron{}
	for _, cron := range crons {
		_cron, err := Insert(db, cron)
		if err != nil {
			return _crons, err
		}
		_crons = append(_crons, _cron)
	}
	return _crons, nil
}
