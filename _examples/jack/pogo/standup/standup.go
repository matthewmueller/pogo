...
var _ pogo.Row = (*Standup)(nil)

// Decode implements pogo.Row
func (s *Standup) Decode(standup interface{}) error {
	_standup, ok := standup.(*Standup)
	if !ok {
		return fmt.Errorf(`standup decode: "%T" is not a "*Standup"`, standup)
	}
	*_standup = *s
	return nil
}
...
func (s *Input) Insert(db pogo.DB) (pogo.Row, error) {
	return Insert(db, s)
}
...
func (f *Filter) ChannelNotIn(v ...string) *Filter {
	...
	f.formats = append(f.formats, fmt.Sprintf(`"channel" NOT IN (%s)`, strings.Join(rs, `, `)))
	for _, i := range v {
		f.values = append(f.values, i)
	}
	return f
...
// InsertMany "standups" into the "jack"."standups"
func InsertMany(db pogo.DB, standups ...*Input) ([]*Standup, error) {
	_standups := []*Standup{}
	for _, standup := range standups {
		_standup, err := Insert(db, standup)
		if err != nil {
			return _standups, err
		}
		_standups = append(_standups, _standup)
	}
	return _standups, nil
}
...
// FindByNameAndTeamID find a standup by name and team_id
func FindByNameAndTeamID(db pogo.DB, name string, teamID int) (*Standup, error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
    SELECT "id", "team_id", "name", "channel", "time", "timezone"
    FROM "jack"."standups"
  ...
  `
	...
	// log query if we've provided a logger
	if pogo.Log != nil {
		pogo.Log(sqlstr, name, teamID)
	}

	var _standup Standup
	row := db.QueryRow(sqlstr, name, teamID)
	err := row.Scan(&_standup.ID, &_standup.TeamID, &_standup.Name, &_standup.Channel, &_standup.Time, &_standup.Timezone)
	if err != nil {
		if err == pgx.ErrNoRows {
			return nil, ErrStandupNotFound
...
// Update "standups" rows in "jack"."standups" by a condition, returning 1 result
func Update(db pogo.DB, standup *Input, conds ...pogo.Condition) (*Standup, error) {
	_s, _v, err := pogo.Conditions(conds...)
	if err != nil {
		return nil, err
	}
	...
	if len(_u) == 0 {
		return nil, errors.New("standup.Update: no input provided")
	}
...
// UpdateMany updates "standups" rows in "jack"."standups" by conditions, returning all results
func UpdateMany(db pogo.DB, standup *Input, conds ...pogo.Condition) ([]*Standup, error) {
	standups := []*Standup{}
	...
	// return an error if no update input is provided
	if len(_u) == 0 {
		return nil, errors.New("standup.UpdateMany: no input provided")
	}
...
func UpdateByID(db pogo.DB, id int, standup *Input) (*Standup, error) {
	...
	// return an error if no update input is provided
	if len(_u) == 0 {
		return nil, errors.New("standup.UpdateByID: no input provided")
	}

	// sql query
	sqlstr := `UPDATE "jack"."standups" ` +
		`SET ` + strings.Join(_u, ", ") + ` ` +
		`WHERE "id" = $1 ` +
		`RETURNING "id", "team_id", "name", "channel", "time", "timezone"`
...
func UpdateByChannel(db pogo.DB, channel string, standup *Input) (*Standup, error) {
	...
	// return an error if no update input is provided
	if len(_u) == 0 {
		return nil, errors.New("standup.UpdateByChannel: no input provided")
	}
...
// UpdateByNameAndTeamID find a Standup
func UpdateByNameAndTeamID(db pogo.DB, name string, teamID int, standup *Input) (*Standup, error) {
	// add values to input, overriding existing keys if present in the input
	standup = standup.TeamID(teamID).Name(name)
	...
	// return an error if no update input is provided
	if len(_u) == 0 {
		return nil, errors.New("standup.UpdateByNameAndTeamID: no input provided")
	}
...
// DeleteByNameAndTeamID deletes a "standup"
func DeleteByNameAndTeamID(db pogo.DB, name string, teamID int) (*Standup, error) {
	// sql delete query
	sqlstr := `DELETE FROM "jack"."standups" ` +
		`WHERE "name" = $1 AND "team_id" = $2 ` +
		`RETURNING "id", "team_id", "name", "channel", "time", "timezone"`
...
// UpsertByID inserts a `standups`, updating the row if `ID` already exists.
func UpsertByID(db pogo.DB, id int, standup *Input) (*Standup, error) {
	// add values to input, overriding existing keys if present in the input
	standup = standup.ID(id)
	...
	// return an error if no update input is provided
	if len(_u) == 0 {
		return nil, errors.New("standup.UpsertByID: no input provided")
	}
...
// UpsertByChannel inserts a `standups`, updating the row if `"channel"` already exists.
...
// UpsertByNameAndTeamID inserts a `standups`, updating the row if `"name", "team_id"` already exists.
func UpsertByNameAndTeamID(db pogo.DB, name string, teamID int, standup *Input) (*Standup, error) {
	// add values to input, overriding existing keys if present in the input
	standup = standup.TeamID(teamID).Name(name)
