...
var _ pogo.Row = (*Question)(nil)

// Decode implements pogo.Row
func (q *Question) Decode(question interface{}) error {
	_question, ok := question.(*Question)
	if !ok {
		return fmt.Errorf(`question decode: "%T" is not a "*Question"`, question)
	}
	*_question = *q
	return nil
}
...
func (q *Input) Insert(db pogo.DB) (pogo.Row, error) {
	return Insert(db, q)
}
...
func Insert(db pogo.DB, question *Input) (*Question, error) {
    ...
    RETURNING "id", "standup_id", "order", "question"
  `
	...
	if e := row.Scan(&_question.ID, &_question.StandupID, &_question.Order, &_question.Question); e != nil {
		return nil, e
	}
...
// InsertMany "questions" into the "jack"."questions"
func InsertMany(db pogo.DB, questions ...*Input) ([]*Question, error) {
	_questions := []*Question{}
	for _, question := range questions {
		_question, err := Insert(db, question)
		if err != nil {
			return _questions, err
		}
...
func Find(db pogo.DB, conds ...pogo.Condition) (*Question, error) {
	...
	sqlstr := `SELECT "id", "standup_id", "order", "question" ` +
		`FROM "jack"."questions" ` +
		_s
...
func FindByID(db pogo.DB, id int) (*Question, error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
    SELECT "id", "standup_id", "order", "question"
    FROM "jack"."questions"
    WHERE "id" = $1
  `
...
// FindByOrderAndStandupID find a question by order and standup_id
func FindByOrderAndStandupID(db pogo.DB, order int, standupID int) (*Question, error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
    SELECT "id", "standup_id", "order", "question"
    FROM "jack"."questions"
    WHERE "order" = $1 AND "standup_id" = $2
  `

	// log query if we've provided a logger
	if pogo.Log != nil {
		pogo.Log(sqlstr, order, standupID)
	}

	var _question Question
	row := db.QueryRow(sqlstr, order, standupID)
	err := row.Scan(&_question.ID, &_question.StandupID, &_question.Order, &_question.Question)
	if err != nil {
...
func FindMany(db pogo.DB, conds ...pogo.Condition) ([]*Question, error) {
	...
	for rows.Next() {
		var _question Question
		if e := rows.Scan(&_question.ID, &_question.StandupID, &_question.Order, &_question.Question); e != nil {
			if e == pgx.ErrNoRows {
...
// UpdateByOrderAndStandupID find a Question
func UpdateByOrderAndStandupID(db pogo.DB, order int, standupID int, question *Input) (*Question, error) {
	// add values to input, overriding existing keys if present in the input
	question = question.StandupID(standupID).Order(order)
	...
	// return an error if no update input is provided
	if len(_u) == 0 {
		return nil, errors.New("question.UpdateByOrderAndStandupID: no input provided")
	}
...
// DeleteByOrderAndStandupID deletes a "question"
func DeleteByOrderAndStandupID(db pogo.DB, order int, standupID int) (*Question, error) {
	// sql delete query
	sqlstr := `DELETE FROM "jack"."questions" ` +
		`WHERE "order" = $1 AND "standup_id" = $2 ` +
		`RETURNING "id", "standup_id", "order", "question"`
...
func UpsertByID(db pogo.DB, id int, question *Input) (*Question, error) {
	...
	var _question Question
	row := db.QueryRow(sqlstr, _v...)
	if e := row.Scan(&_question.ID, &_question.StandupID, &_question.Order, &_question.Question); e != nil && e != pgx.ErrNoRows {
		return nil, e
	}
...
// UpsertByOrderAndStandupID inserts a `questions`, updating the row if `"order", "standup_id"` already exists.
func UpsertByOrderAndStandupID(db pogo.DB, order int, standupID int, question *Input) (*Question, error) {
	// add values to input, overriding existing keys if present in the input
	question = question.StandupID(standupID).Order(order)
