...
var _ pogo.Row = (*Report)(nil)

// Decode implements pogo.Row
func (r *Report) Decode(report interface{}) error {
	_report, ok := report.(*Report)
	if !ok {
		return fmt.Errorf(`report decode: "%T" is not a "*Report"`, report)
	}
	*_report = *r
	return nil
}
...
func (r *Input) Insert(db pogo.DB) (pogo.Row, error) {
	return Insert(db, r)
}
...
// InsertMany "reports" into the "jack"."reports"
func InsertMany(db pogo.DB, reports ...*Input) ([]*Report, error) {
	_reports := []*Report{}
	for _, report := range reports {
		_report, err := Insert(db, report)
		if err != nil {
			return _reports, err
		}
		_reports = append(_reports, _report)
	}
	return _reports, nil
}
