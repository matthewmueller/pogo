...
var _ pogo.Row = (*Answer)(nil)

// Decode implements pogo.Row
func (a *Answer) Decode(answer interface{}) error {
	_answer, ok := answer.(*Answer)
	if !ok {
		return fmt.Errorf(`answer decode: "%T" is not a "*Answer"`, answer)
	}
	*_answer = *a
	return nil
}
...
func (a *Input) Insert(db pogo.DB) (pogo.Row, error) {
	return Insert(db, a)
}
...
func (f *Filter) QuestionIDIn(v ...int) *Filter {
	...
	f.formats = append(f.formats, fmt.Sprintf(`"question_id" IN (%s)`, strings.Join(rs, `, `)))
	for _, i := range v {
		f.values = append(f.values, i)
	}
	return f
...
func (f *Filter) QuestionIDNotIn(v ...int) *Filter {
	...
	f.formats = append(f.formats, fmt.Sprintf(`"question_id" NOT IN (%s)`, strings.Join(rs, `, `)))
	for _, i := range v {
		f.values = append(f.values, i)
	}
	return f
...
func (f *Filter) AnswerNotIn(v ...string) *Filter {
	...
	f.formats = append(f.formats, fmt.Sprintf(`"answer" NOT IN (%s)`, strings.Join(rs, `, `)))
	for _, i := range v {
		f.values = append(f.values, i)
	}
	return f
...
// InsertMany "answers" into the "jack"."answers"
func InsertMany(db pogo.DB, answers ...*Input) ([]*Answer, error) {
	_answers := []*Answer{}
	for _, answer := range answers {
		_answer, err := Insert(db, answer)
		if err != nil {
			return _answers, err
		}
		_answers = append(_answers, _answer)
	}
	return _answers, nil
}
...
// Update "answers" rows in "jack"."answers" by a condition, returning 1 result
func Update(db pogo.DB, answer *Input, conds ...pogo.Condition) (*Answer, error) {
	_s, _v, err := pogo.Conditions(conds...)
	if err != nil {
		return nil, err
	}
	...
	if len(_u) == 0 {
		return nil, errors.New("answer.Update: no input provided")
	}
...
// UpdateMany updates "answers" rows in "jack"."answers" by conditions, returning all results
func UpdateMany(db pogo.DB, answer *Input, conds ...pogo.Condition) ([]*Answer, error) {
	answers := []*Answer{}
	...
	// return an error if no update input is provided
	if len(_u) == 0 {
		return nil, errors.New("answer.UpdateMany: no input provided")
	}
...
func UpdateByID(db pogo.DB, id int, answer *Input) (*Answer, error) {
	...
	// return an error if no update input is provided
	if len(_u) == 0 {
		return nil, errors.New("answer.UpdateByID: no input provided")
	}

	// sql query
	sqlstr := `UPDATE "jack"."answers" ` +
		`SET ` + strings.Join(_u, ", ") + ` ` +
		`WHERE "id" = $1 ` +
		`RETURNING "id", "question_id", "teammate_id", "answer"`
...
// UpsertByID inserts a `answers`, updating the row if `ID` already exists.
func UpsertByID(db pogo.DB, id int, answer *Input) (*Answer, error) {
	...
	// return an error if no update input is provided
	if len(_u) == 0 {
		return nil, errors.New("answer.UpsertByID: no input provided")
	}

	// sql query
	sqlstr := `INSERT INTO "jack"."answers" (` + strings.Join(_c, ", ") + `) ` +
		`VALUES (` + strings.Join(_i, ", ") + `) ` +
		`ON CONFLICT (id) ` +
	...
	var _answer Answer
	row := db.QueryRow(sqlstr, _v...)
	if e := row.Scan(&_answer.ID, &_answer.QuestionID, &_answer.TeammateID, &_answer.Answer); e != nil && e != pgx.ErrNoRows {
		return nil, e
	}
