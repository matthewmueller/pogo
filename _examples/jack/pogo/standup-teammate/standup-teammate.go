...
type StandupTeammate struct {
	...
	Status     enum.StandupTeammateStatus `json:"status,omitempty"`
	Time       string                     `json:"time,omitempty"`
	Owner      bool                       `json:"owner,omitempty"`
}

var _ pogo.Row = (*StandupTeammate)(nil)

// Decode implements pogo.Row
func (st *StandupTeammate) Decode(standupTeammate interface{}) error {
	_standupTeammate, ok := standupTeammate.(*StandupTeammate)
	if !ok {
		return fmt.Errorf(`standupTeammate decode: "%T" is not a "*StandupTeammate"`, standupTeammate)
	}
	*_standupTeammate = *st
	return nil
}
...
type Input struct {
	...
	standupID  *int
	teammateID *int
	status     *enum.StandupTeammateStatus
	time       *string
	owner      *bool
}
...
func (st *Input) Status(status enum.StandupTeammateStatus) *Input {
	st.status = &status
	return st
}
...
func (st *Input) Insert(db pogo.DB) (pogo.Row, error) {
	return Insert(db, st)
}
...
func (f *Filter) Status(v enum.StandupTeammateStatus) *Filter {
...
func (f *Filter) StatusNot(v enum.StandupTeammateStatus) *Filter {
	f.formats = append(f.formats, ``)
	f.values = append(f.values, v)
	return f
}
...
func (f *Filter) StatusIn(v ...enum.StandupTeammateStatus) *Filter {
	var rs []string
	for range v {
		rs = append(rs, "%s")
	}
...
func (f *Filter) StatusNotIn(v ...enum.StandupTeammateStatus) *Filter {
	var rs []string
	for range v {
		rs = append(rs, "%s")
	}
...
func Insert(db pogo.DB, standupTeammate *Input) (*StandupTeammate, error) {
    ...
    RETURNING "id", "standup_id", "teammate_id", "status", "time", "owner"
  `
	...
	if e := row.Scan(&_standupTeammate.ID, &_standupTeammate.StandupID, &_standupTeammate.TeammateID, &_standupTeammate.Status, &_standupTeammate.Time, &_standupTeammate.Owner); e != nil {
		return nil, e
	}
...
// InsertMany "standups_teammates" into the "jack"."standups_teammates"
func InsertMany(db pogo.DB, standupsTeammates ...*Input) ([]*StandupTeammate, error) {
	_standupsTeammates := []*StandupTeammate{}
	for _, standupTeammate := range standupsTeammates {
		_standupTeammate, err := Insert(db, standupTeammate)
		if err != nil {
			return _standupsTeammates, err
		}
		_standupsTeammates = append(_standupsTeammates, _standupTeammate)
	}
	return _standupsTeammates, nil
}
...
func Find(db pogo.DB, conds ...pogo.Condition) (*StandupTeammate, error) {
	...
	sqlstr := `SELECT "id", "standup_id", "teammate_id", "status", "time", "owner" ` +
...
func FindByID(db pogo.DB, id int) (*StandupTeammate, error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
    SELECT "id", "standup_id", "teammate_id", "status", "time", "owner"
    FROM "jack"."standups_teammates"
    WHERE "id" = $1
  `
...
func FindByStandupIDAndTeammateID(db pogo.DB, standupID int, teammateID int) (*StandupTeammate, error) {
  ...
  `
	...
	err := row.Scan(&_standupTeammate.ID, &_standupTeammate.StandupID, &_standupTeammate.TeammateID, &_standupTeammate.Status, &_standupTeammate.Time, &_standupTeammate.Owner)
	if err != nil {
...
func FindMany(db pogo.DB, conds ...pogo.Condition) ([]*StandupTeammate, error) {
	standupsTeammates := []*StandupTeammate{}
	...
	for rows.Next() {
		var _standupTeammate StandupTeammate
		if e := rows.Scan(&_standupTeammate.ID, &_standupTeammate.StandupID, &_standupTeammate.TeammateID, &_standupTeammate.Status, &_standupTeammate.Time, &_standupTeammate.Owner); e != nil {
			if e == pgx.ErrNoRows {
...
func UpsertByID(db pogo.DB, id int, standupTeammate *Input) (*StandupTeammate, error) {
	...
	var _standupTeammate StandupTeammate
	row := db.QueryRow(sqlstr, _v...)
	if e := row.Scan(&_standupTeammate.ID, &_standupTeammate.StandupID, &_standupTeammate.TeammateID, &_standupTeammate.Status, &_standupTeammate.Time, &_standupTeammate.Owner); e != nil && e != pgx.ErrNoRows {
		return nil, e
	}
