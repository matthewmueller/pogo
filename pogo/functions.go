...
var templateMap = template.FuncMap{
	...
	"mcapitalize": func() func(string) string {
		return func(s string) string {
			return snaker.SnakeToCamelIdentifier(s)
		}
	},

	"mcamelize": func() func(string) string {
		return func(s string) string {
			return camelize(s)
		}
	},
	...
	// lower string
	"mlower": func() func(string) string {
		return func(s string) string {
			return strings.ToLower(s)
		}
	},
	...
	"colnames": func(columns []*Column) (cols []string) {
		for _, col := range columns {
	...
	"idxcolnames": func(index *Index) (cols []string) {
	...
	// this could be composed, but it'd be a pita
	"idxparams": func(schema *Schema, index *Index) string {
		cols := []string{}
	...
	"mprintf": func(format string) func(string) string {
		return func(s string) string {
	...
	"tablename": func(schema *Schema, table *Table) string {
		s := schema.Name
		t := table.Name
...
// 	cols := []string{}
// 	for _, col := range columns {
// 		cols = append(cols, "&"+key+"."+f.field(col.Name))
// 	}
// 	return strings.Join(cols, ", ")
// }
...
// 			cols = append(cols, f.field(col.Name))
// 		}
// 	}
...
// 	for _, col := range index.Columns {
// 		if index.IsUnique && !index.IsPrimary {
// 			cols = append(cols, strings.ToLower(f.field(col.Name))+" "+f.Coerce.Coerce(col.DataType))
// 		}
// 	}
...
// 	for _, col := range index.Columns {
// 		if index.IsUnique && !index.IsPrimary {
// 			cols = append(cols, strings.ToLower(f.field(col.Name)))
// 		}
// 	}
...
// // 		return a.Loader.Escape(ColumnEsc, col.Name)
// // 	}
// //
// // 	return col.Name
// // }
