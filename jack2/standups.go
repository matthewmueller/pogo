...
func (standups *Standups) FindByID(iD *uuid.UUID) (standup *Standup, err error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
	SELECT "id", "name", "slack_channel_id", "time", "timezone", "questions", "team_id", "created_at", "updated_at"
	FROM jack.standups
...
// UpdateByID find a Standup
func (standups *Standups) UpdateByID(standup Standup, iD *uuid.UUID) (*Standup, error) {
	fieldset := standups.fields(&standup)
...
// DeleteByID find a Standup
func (standups *Standups) DeleteByID(iD *uuid.UUID) error {
	// sql delete query
	sqlstr := `DELETE FROM jack.standups WHERE "id" = $1`
...
// UpsertByID find a Standup
func (standups *Standups) UpsertByID(standup Standup, action string) (*Standup, error) {
	// get all the non-nil fields and prepare them for the query
	_c, _i, _v := slice(standups.fields(&standup), 0)
