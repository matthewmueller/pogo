...
// ErrProjectNotFound returned if the project is not found
...
func Insert(db testdigby.DB, projectInput *ProjectInput) (*Project, error) {
	// get all the non-nil columns and prepare them for the query
	_c, _i, _v := testdigby.Slice(getColumns(projectInput), 0)
...
func FindByID(db testdigby.DB, id string) (*Project, error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
    SELECT "id", "name", "domain", "created_at", "updated_at"
    FROM "public.projects"
...
func FindOne(db testdigby.DB, where *WhereClause) (*Project, error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
  SELECT "id", "name", "domain", "created_at", "updated_at"
  FROM "public.projects"
...
func FindMany(db testdigby.DB, where *WhereClause) (projects []*Project, err error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
  SELECT "id", "name", "domain", "created_at", "updated_at"
  FROM "public.projects"
...
func Update(db testdigby.DB, id string, projectInput *ProjectInput) (*Project, error) {
...
func UpdateMany(db testdigby.DB, where *WhereClause, projectInput *ProjectInput) (projects []*Project, err error) {
	// prepare the slices
	_c, _i, _v := testdigby.Slice(getColumns(projectInput), len(where.params))
...
func Upsert(db testdigby.DB, projectInput *ProjectInput) (*Project, error) {
	// get all the non-nil columns and prepare them for the query
	_c, _i, _v := testdigby.Slice(getColumns(projectInput), 0)
