...
// ErrShortidNotFound returned if the shortid is not found
var ErrShortidNotFound = errors.New("shortid not found")
...
func Insert(db testdigby.DB, shortidInput *ShortidInput) (*Shortid, error) {
	// get all the non-nil columns and prepare them for the query
	_c, _i, _v := testdigby.Slice(getColumns(shortidInput), 0)
...
func FindByID(db testdigby.DB, id string) (*Shortid, error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
    SELECT "id", "created_at"
    FROM "public.shortids"
...
func FindOne(db testdigby.DB, where *WhereClause) (*Shortid, error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
  SELECT "id", "created_at"
  FROM "public.shortids"
...
func FindMany(db testdigby.DB, where *WhereClause) (shortids []*Shortid, err error) {
	// sql select query, primary key provided by sequence
	sqlstr := `
  SELECT "id", "created_at"
  FROM "public.shortids"
...
func Update(db testdigby.DB, id string, shortidInput *ShortidInput) (*Shortid, error) {
...
func UpdateMany(db testdigby.DB, where *WhereClause, shortidInput *ShortidInput) (shortids []*Shortid, err error) {
	// prepare the slices
	_c, _i, _v := testdigby.Slice(getColumns(shortidInput), len(where.params))
...
func Upsert(db testdigby.DB, shortidInput *ShortidInput) (*Shortid, error) {
	// get all the non-nil columns and prepare them for the query
	_c, _i, _v := testdigby.Slice(getColumns(shortidInput), 0)
